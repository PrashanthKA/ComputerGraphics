...
class cl_widgets:
    ...
    def __init__(self,ob_root_window,ob_world=[]):
        ...
        self.readCameraFile()
        self.toolbar.plotViewPort(self.cameraObjects)

    def readCameraFile(self):
        # Load camera file:
        fl_obj = open("cameras_04.txt",'r')
        firstCamera = 0
        tempCameraObject = cameraClass(self,self.ob_root_window)
        for line in fl_obj:
                if line[0] == 'c':
...
class cl_flyToolBarWindow:
    def __init__(self,master,ob_root_window):
        ...
        var = StringVar()
        var.set('front')
        choices = ['front', 'side', 'top', 'perspective1','perspective2']
        option = OptionMenu(fram1, var, *choices)
        option.pack(side='left', padx=10, pady=10)
        option.grid(row=0,column=1,sticky=W)
        Label(fram1, text="VRP1").grid(row=0,column=2, sticky=W)
        fl1 = StringVar()
        fl1.set(0.0)
    ...
    def makeItFly(self):
        ...
        vrp1 = [VRPpoint1x,VRPpoint1y,VRPpoint1z]
        ...
        if(whichView == "front"):
            for x in range(steps+1):
                ...
                VRPVector2 = [vrp1[0]+((x)*xdiff),vrp1[1]+((x)*ydiff),vrp1[2]+((x)*zdiff)]
                self.master.cameraObjects[0].cVRP = VRPVector2
                self.master.ob_world.doMagic(self.master.ob_canvas_frame.canvas)
                self.master.ob_canvas_frame.canvas.update()
        elif(whichView == "side"):
            for x in range(steps+1):
                self.master.ob_canvas_frame.canvas.delete('all')
                VRPVector2 = [vrp1[0]+((x)*xdiff),vrp1[1]+((x)*ydiff),vrp1[2]+((x)*zdiff)]
                self.master.cameraObjects[1].cVRP = VRPVector2
                self.master.ob_world.doMagic(self.master.ob_canvas_frame.canvas)
                self.master.ob_canvas_frame.canvas.update()
        elif(whichView == "top"):
            for x in range(steps+1):
                self.master.ob_canvas_frame.canvas.delete('all')
                VRPVector2 = [vrp1[0]+((x)*xdiff),vrp1[1]+((x)*ydiff),vrp1[2]+((x)*zdiff)]
                self.master.cameraObjects[2].cVRP = VRPVector2
                self.master.ob_world.doMagic(self.master.ob_canvas_frame.canvas)
                self.master.ob_canvas_frame.canvas.update()
        elif(whichView == "perspective1"):
            for x in range(steps+1):
                self.master.ob_canvas_frame.canvas.delete('all')
                VRPVector2 = [vrp1[0]+((x)*xdiff),vrp1[1]+((x)*ydiff),vrp1[2]+((x)*zdiff)]
                self.master.cameraObjects[3].cVRP = VRPVector2
                self.master.ob_world.doMagic(self.master.ob_canvas_frame.canvas)
                self.master.ob_canvas_frame.canvas.update()
        elif(whichView == "perspective2"):
            for x in range(steps+1):
                self.master.ob_canvas_frame.canvas.delete('all')
                VRPVector2 = [vrp1[0]+((x)*xdiff),vrp1[1]+((x)*ydiff),vrp1[2]+((x)*zdiff)]
                self.master.cameraObjects[4].cVRP = VRPVector2
                self.master.ob_world.doMagic(self.master.ob_canvas_frame.canvas)
                self.master.ob_canvas_frame.canvas.update()
...
class cl_pannel_02:
    ...
    def __init__(self, master):
        ...
        frame = Frame(master.ob_root_window)
        frame.pack()
        self.button = Button(frame, text="Fly", fg="blue", command=self.open_dialog_callback)
        self.button.pack(side=LEFT)
...
class MyDialog(simpledialog.Dialog):
    def body(self, master):
        ...
        Label(master, text="Fy:").grid(row=0,column=0, sticky=W)
        var = StringVar()
        var.set('front')
        choices = ['front', 'side', 'top', 'perspective1','perspective2']
        ...
        Label(master, text="VRP1").grid(row=0,column=2, sticky=W)
        fl1 = StringVar()
        fl1.set(0.0)
        e = Entry(master, textvariable=fl1,width=5)
        e.grid(row=0,column=3)
        fl2 = StringVar()
        fl2.set(0.0)
        e = Entry(master, textvariable=fl2,width=5)
        e.grid(row=0,column=4)
        fl3 = StringVar()
        fl3.set(0.0)
        e = Entry(master, textvariable=fl3,width=5)
        e.grid(row=0,column=6)

        Label(master, text="VRP2").grid(row=0,column=7, sticky=W)
        pl1 = StringVar()
        pl1.set(1.0)
        e = Entry(master, textvariable=pl1,width=5)
        e.grid(row=0,column=8)
        pl2 = StringVar()
        pl2.set(1.0)
        e = Entry(master, textvariable=pl2,width=5)
        e.grid(row=0,column=9)
        pl3 = StringVar()
        pl3.set(1.0)
        e = Entry(master, textvariable=pl3,width=5)
        e.grid(row=0,column=10)


        Label(master, text="Steps:").grid(row=0,column=11, sticky=W)

        fl4 = StringVar()
        fl4.set(1)
        e = Entry(master, textvariable=fl4,width=5)
        e.grid(row=0,column=12)


    def apply(self):
        try:
            vrp1 = [float(fl1.get()),float(fl2.get()),float(fl3.get())]
            vrp2 = [float(pl1.get()),float(pl2.get()),float(pl3.get())]
            steps = int(fl4.get())
            xdiff = (vrp2[0]-vrp1[0])/steps
            ydiff = (vrp2[1]-vrp1[1])/steps
            zdiff = (vrp2[2]-vrp1[2])/steps
            whichView = var.get()
            if(whichView == "front"):
                for x in range(steps+1):
                    VRPVector2 = [vrp1[0]+((x)*xdiff),vrp1[1]+((x)*ydiff),vrp1[2]+((x)*zdiff)]
                    self.master.master.cameraObjects[0].cVRP = VRPVector2
                    self.master.ob_world.doMagic(self.master.ob_canvas_frame.canvas)
                    self.master.ob_canvas_frame.canvas.update()
...
class cl_toolbar:
    ...
    def plotViewPort(self, cameraObjectList=[]):
        self.master.ob_world.plotViewPorts(self.master.ob_canvas_frame.canvas,cameraObjectList)
    ...
    def toolbar_callback(self):
        ...
        if(fileToBeRead):
            ...
            self.master.ob_world.updateVectorFaces(mainVectorList,mainFaceList)
            self.master.ob_world.doMagic(self.master.ob_canvas_frame.canvas)
